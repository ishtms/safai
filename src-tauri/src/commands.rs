...
use crate::onboarding::{
    self, OnboardingState, OnboardingStep, OnboardingStore, PermissionKind, PermissionStatus,
    Preferences,
};
use crate::scanner::activity::{
    self, kill_pid as activity_kill_pid, next_activity_handle_id, retop_snapshot,
    run_activity_stream, sample_activity, ActivityController, ActivityEmit, ActivityHandle,
    ActivityInsert, ActivityRegistry, ActivitySnapshot, ProcessRow, SysinfoProbe,
};
use crate::scanner::dupes::{
    self, next_dupes_handle_id, run_dupes_stream, scan_duplicates, DupesController, DupesEmit,
    DupesHandle, DupesInsert, DupesRegistry, DuplicateReport,
};
...
use crate::scanner::largeold::{
    ...
    LargeOldEmit, LargeOldHandle, LargeOldInsert, LargeOldRegistry, LargeOldReport,
};
use crate::scanner::malware::{
    self, next_malware_handle_id, run_malware_stream, scan_malware, MalwareController, MalwareEmit,
    MalwareHandle, MalwareInsert, MalwareOptions, MalwareRegistry, MalwareReport,
};
use crate::scanner::privacy::{self, PrivacyReport};
use crate::scanner::run::{
    next_handle_id, run_scan, Emit, ScanController, ScanEvent, ScanInsert, ScanProgress,
    ScanRegistry, ScanRoot, ScanState, VolumeSnapshot,
};
use crate::scanner::startup::{self, StartupReport, StartupSource, ToggleResult};
...
use crate::scanner::treemap::{
    ...
    TreemapController, TreemapEmit, TreemapHandle, TreemapInsert, TreemapRegistry, TreemapResponse,
};
use crate::scanner::{
...
struct IpcEventEnvelope<'a, T: Serialize> {
    kind: &'static str,
    handle_id: &'a str,
...
impl<'a, T: Serialize> Clone for IpcEventEnvelope<'a, T> {
    fn clone(&self) -> Self {
        Self {
            kind: self.kind,
...
impl Emit for AppEmitter {
    ...
    fn emit_done(&self, handle_id: &str, p: &ScanProgress) {
        ...
        let registry = self.app.state::<ScanRegistry>();
        registry.finish(handle_id);
        emit_enveloped(
            &self.app,
            EVENT_SCAN_DONE,
            "done",
...
pub fn start_scan(
    ...
    let scan_roots: Vec<ScanRoot> = roots
        ...
        .map(|v| {
            v.into_iter()
                .map(|s| ScanRoot::user(PathBuf::from(s)))
                .collect()
        })
    ...
    match registry.insert_or_active(id.clone(), ctrl.clone(), roots_echo.clone()) {
        ScanInsert::Inserted => {}
        ScanInsert::Active { id, roots } => {
            eprintln!("[safai] start_scan reused active handle {id}");
            return Ok(ScanHandle { id, roots });
        }
    }

    let emitter = AppEmitter::new(app.clone());
...
#[tauri::command]
pub fn scan_snapshot(registry: State<'_, ScanRegistry>, handle_id: String) -> Option<ScanProgress> {
...
impl MalwareEmit for AppMalwareEmitter {
    fn emit_progress(&self, id: &str, resp: &MalwareReport) {
        emit_enveloped(
            &self.app,
            EVENT_MALWARE_PROGRESS,
            "progress",
            id,
            Some(resp.phase.as_str()),
            self.next_sequence(),
            false,
            resp,
        );
    }
    fn emit_done(&self, id: &str, resp: &MalwareReport) {
        let registry = self.app.state::<MalwareRegistry>();
        registry.finish(id, resp.clone());
        emit_enveloped(
            &self.app,
            EVENT_MALWARE_DONE,
            "done",
...
pub fn start_malware(
    ...
    let handle = MalwareHandle {
        id: id.clone(),
        platform: crate::scanner::malware::types::platform_tag(os).to_string(),
    };
    match registry.insert_or_active(handle.clone(), ctrl.clone()) {
        MalwareInsert::Inserted => {}
        MalwareInsert::Active(active) => {
            eprintln!("[safai] start_malware reused active handle {}", active.id);
            return Ok(active);
        }
    }

    let emitter = AppMalwareEmitter::new(app.clone());
    let id_thread = id.clone();
    let spawn = std::thread::Builder::new()
    ...
    if let Err(e) = spawn {
        let _ = registry.remove(&id);
        return Err(format!("failed to spawn malware thread: {e}"));
    }
...
#[tauri::command]
pub fn malware_snapshot(
    registry: State<'_, MalwareRegistry>,
    handle_id: String,
) -> Option<MalwareReport> {
    registry.terminal_snapshot(&handle_id)
}

// ---------------- safe deletion engine ----------------

/// process-wide cleaner. called once from setup and managed by tauri.
/// also runs a TTL sweep, failures log but don't block startup.
pub fn build_cleaner(app: &AppHandle) -> Result<Cleaner, String> {
    let home = home_dir().ok_or_else(|| {
        "could not resolve user home directory for cleaner safety checks".to_string()
    })?;
    let data = app
        .path()
        .app_data_dir()
        .map_err(|e| format!("could not resolve app data directory for cleaner: {e}"))?;
...
impl TreemapEmit for AppTreemapEmitter {
    fn emit_progress(&self, id: &str, resp: &TreemapResponse) {
        emit_enveloped(
            &self.app,
            EVENT_TREEMAP_PROGRESS,
            "progress",
            id,
            None,
            self.next_sequence(),
    ...
    fn emit_done(&self, id: &str, resp: &TreemapResponse) {
        let registry = self.app.state::<TreemapRegistry>();
        registry.finish(id, resp.clone());
        emit_enveloped(
            &self.app,
            EVENT_TREEMAP_DONE,
...
/// latest. Events still carry the handle id so stale listeners cannot
/// accept another walk's snapshots.
#[tauri::command]
pub fn start_treemap(
    ...
    if let Some(active) = registry.active_handle() {
        eprintln!("[safai] start_treemap reused active handle {}", active.id);
        return Ok(active);
    }
    ...
    if let Err(e) = spawn {
        let _ = registry.remove(&id);
        return Err(format!("failed to spawn treemap thread: {e}"));
    }
...
#[tauri::command]
pub fn treemap_snapshot(
    registry: State<'_, TreemapRegistry>,
    handle_id: String,
) -> Option<TreemapResponse> {
    registry.terminal_snapshot(&handle_id)
}
...
impl DupesEmit for AppDupesEmitter {
    fn emit_progress(&self, id: &str, resp: &DuplicateReport) {
        emit_enveloped(
            &self.app,
            EVENT_DUPES_PROGRESS,
            "progress",
            id,
            Some(resp.phase.as_str()),
            self.next_sequence(),
    ...
    fn emit_done(&self, id: &str, resp: &DuplicateReport) {
        let registry = self.app.state::<DupesRegistry>();
        registry.finish(id, resp.clone());
        emit_enveloped(
            &self.app,
            EVENT_DUPES_DONE,
...
pub fn start_duplicates(
    ...
    if let Some(active) = registry.active_handle() {
        eprintln!(
            "[safai] start_duplicates reused active handle {}",
            active.id
        );
        return Ok(active);
    }
    ...
    let emitter = AppDupesEmitter::new(app.clone());
    let id_thread = id.clone();
    let spawn = std::thread::Builder::new()
    ...
    if let Err(e) = spawn {
        let _ = registry.remove(&id);
        return Err(format!("failed to spawn dupes thread: {e}"));
    }
...
impl LargeOldEmit for AppLargeOldEmitter {
    fn emit_progress(&self, id: &str, resp: &LargeOldReport) {
        emit_enveloped(
            &self.app,
            EVENT_LARGE_OLD_PROGRESS,
            "progress",
            id,
            Some(resp.phase.as_str()),
            self.next_sequence(),
    ...
    fn emit_done(&self, id: &str, resp: &LargeOldReport) {
        let registry = self.app.state::<LargeOldRegistry>();
        registry.finish(id, resp.clone());
        emit_enveloped(
            &self.app,
            EVENT_LARGE_OLD_DONE,
            "done",
            id,
            Some(resp.phase.as_str()),
            self.next_sequence(),
...
pub fn start_large_old(
    ...
    if let Some(active) = registry.active_handle() {
        eprintln!("[safai] start_large_old reused active handle {}", active.id);
        return Ok(active);
    }
    ...
    match registry.insert_or_active(handle.clone(), ctrl.clone()) {
        ...
        LargeOldInsert::Active(active) => {
            eprintln!("[safai] start_large_old reused active handle {}", active.id);
            return Ok(active);
        }
    }

    let emitter = AppLargeOldEmitter::new(app.clone());
    let id_thread = id.clone();
    let spawn = std::thread::Builder::new()
    ...
    if let Err(e) = spawn {
        let _ = registry.remove(&id);
        return Err(format!("failed to spawn large-old thread: {e}"));
    }
...
#[tauri::command]
pub fn cancel_large_old(registry: State<'_, LargeOldRegistry>, handle_id: String) -> bool {
    match registry.get(&handle_id) {
        Some(ctrl) => {
            ctrl.cancel();
            true
...
#[tauri::command]
pub fn forget_large_old(registry: State<'_, LargeOldRegistry>, handle_id: String) -> bool {
    registry.remove(&handle_id).is_some()
}

#[tauri::command]
pub fn large_old_snapshot(
    registry: State<'_, LargeOldRegistry>,
    handle_id: String,
) -> Option<LargeOldReport> {
    registry.terminal_snapshot(&handle_id)
}

/// mac: open -R, win: explorer /select,, linux: open containing dir with
/// the first available desktop opener
...
pub fn reveal_in_file_manager(path: String) -> Result<(), String> {
    ...
    #[cfg(all(not(target_os = "macos"), not(target_os = "windows")))]
    {
        ...
        for (bin, prefix_args) in attempts {
            ...
            cmd.args(*prefix_args).arg(&target);
            match cmd.spawn() {
                Ok(_) => return Ok(()),
                Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
                    failures.push(format!("{bin}: not found"));
                }
                Err(e) => failures.push(format!("{bin}: {e}")),
            }
        }
        Err(format!(
            "no supported file opener could reveal {} ({})",
            target.display(),
            failures.join("; ")
        ))
    }
...
/// one-shot. Returns the stream's latest cached snapshot when available.
/// On cold start it kicks the stream and returns an immediate sample without
/// sleeping, so first paint is not blocked on sysinfo's CPU delta window.
#[tauri::command]
pub fn activity_sample(
    app: AppHandle,
    registry: State<'_, ActivityRegistry>,
    top_n: Option<usize>,
) -> Result<ActivitySnapshot, String> {
    let n = top_n.unwrap_or(activity::DEFAULT_TOP_N);
    if let Some(snap) = registry.latest_snapshot() {
        return Ok(retop_snapshot(snap, n));
    }

    if let Err(e) = ensure_activity_stream(app, registry.inner(), None, Some(n)) {
        eprintln!("[safai] activity_sample could not start background stream: {e}");
    }
    if let Some(snap) = registry.latest_snapshot() {
        return Ok(retop_snapshot(snap, n));
    }
...
#[tauri::command]
pub fn activity_process_detail(pid: u32) -> Result<Option<ProcessRow>, String> {
    let mut probe = SysinfoProbe::new();
    Ok(probe.process_detail(pid))
}
...
impl ActivityEmit for AppActivityEmitter {
    fn emit_snapshot(&self, id: &str, snap: &ActivitySnapshot) {
        emit_enveloped(
            &self.app,
...
fn ensure_activity_stream(
    ...
    let emitter = AppActivityEmitter::new(app);
    ...
    match registry.insert_or_active(handle.clone(), ctrl.clone()) {
        ...
        ActivityInsert::Active(mut active) => {
            if let Some(ms) = interval_ms {
                if let Some(active_ctrl) = registry.get(&active.id) {
                    active_ctrl.set_interval_ms(ms);
                    active.interval_ms = active_ctrl.interval_ms();
                }
            }
            eprintln!("[safai] start_activity reused active handle {}", active.id);
            return Ok(active);
        }
    }
    let id_thread = id.clone();
    ...
    if let Err(e) = spawn {
        let _ = registry.remove(&id);
        return Err(format!("failed to spawn activity thread: {e}"));
    }
    Ok(handle)
}
...
pub fn start_activity(
    ...
    ensure_activity_stream(app, registry.inner(), interval_ms, top_n)
}

/// nudge to re-tick without waiting for the next interval. noop
/// interval reset so the cancellable-sleep wakes up.
#[tauri::command]
pub fn refresh_activity(registry: State<'_, ActivityRegistry>, handle_id: String) -> bool {
    match registry.get(&handle_id) {
        Some(ctrl) => {
...
#[tauri::command]
pub fn cancel_activity(registry: State<'_, ActivityRegistry>, handle_id: String) -> bool {
    match registry.remove(&handle_id) {
        Some(ctrl) => {
            ctrl.cancel();
            true
        }
...
#[tauri::command]
pub fn forget_activity(registry: State<'_, ActivityRegistry>, handle_id: String) -> bool {
    registry.remove(&handle_id).is_some()
}
...
/// onboarding.
#[tauri::command]
pub fn onboarding_state(store: State<'_, Arc<OnboardingStore>>) -> OnboardingState {
    store.load()
}

/// full object replace, not a patch
#[tauri::command]
pub fn onboarding_save_prefs(
    store: State<'_, Arc<OnboardingStore>>,
    prefs: Preferences,
) -> Result<OnboardingState, String> {
    store
        .update(|state| state.apply_prefs(prefs))
        .map_err(|e| e.to_string())
}

/// lets relaunch mid-flow resume. does not advance completed_at.
#[tauri::command]
pub fn onboarding_set_step(
    store: State<'_, Arc<OnboardingStore>>,
    step: OnboardingStep,
) -> Result<OnboardingState, String> {
    store
        .update(|state| state.last_step = step)
        .map_err(|e| e.to_string())
}
...
pub fn onboarding_record_permission(
    ...
    let now = now_unix();
    store
        .update(|state| state.record_permission(kind, status, now))
        .map_err(|e| e.to_string())
}
...
/// toggle a checkbox
#[tauri::command]
pub fn onboarding_set_telemetry(
    store: State<'_, Arc<OnboardingStore>>,
    opt_in: bool,
) -> Result<OnboardingState, String> {
    store
        .update(|state| state.telemetry_opt_in = opt_in)
        .map_err(|e| e.to_string())
}
...
pub fn onboarding_complete(
    ...
    let now = now_unix();
    store
        .update(|state| state.mark_complete(now))
        .map_err(|e| e.to_string())
}

/// settings screen "Re-run onboarding" button
#[tauri::command]
pub fn onboarding_reset(store: State<'_, Arc<OnboardingStore>>) -> Result<(), String> {
...
pub fn onboarding_permission_status() -> Vec<PermissionStatusEntry> {
    ...
    onboarding::applicable_for(platform)
        ...
        .map(|kind| PermissionStatusEntry {
            ...
            status: home
                .as_deref()
                .map(|h| onboarding::detect_status(kind, h))
...
fn settings_bundle_from_state(s: OnboardingState, now: u64) -> SettingsBundle {
...
pub fn settings_get(
    ...
    // state.json
    let _ = scheduler.controller.set_cadence(s.prefs.scheduled_scan);
    settings_bundle_from_state(s, now)
}
...
pub fn settings_update(
    ...
    let now = now_unix();
    Ok(settings_bundle_from_state(state, now))
}
...
#[tauri::command]
pub fn scheduler_status(store: State<'_, Arc<OnboardingStore>>) -> SchedulerStatus {
    let s = store.load();
...
pub fn spawn_scheduler(app: AppHandle, store: Arc<OnboardingStore>) -> Scheduler {
...
pub(crate) fn safai_data_dir() -> PathBuf {
    dirs::data_dir()
        .map(|p| p.join("safai"))
        .or_else(|| home_dir().map(|home| cleaner::default_data_dir(&home)))
        .unwrap_or_else(|| std::env::temp_dir().join("safai"))
}
